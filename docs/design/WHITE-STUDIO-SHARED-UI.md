...
1. Hub entrance overlay (`#arcade-intro`) — black title lead (~0.7s), then dip into Canvas2D first-person cable / data-stream loading (~3.2s; layered tube walls, tiered streaks + dust, current-self core, tunnel-vision color wash, exit flare with bloom/chroma/grain quality pass). Phone uses the **same** `WhiteStudioArcadeCurrent` scene with `quality: "phone"` / portrait retune (lower particle counts, DPR cap, safe-area panel); not a separate CSS-only loader. At **21:9 and wider**, height-based focal scaling, wider horizontal projection, denser particles/ribs, and a compact copy safe area prevent empty side bands or center obstruction. Plays on **every** hub load / reconnect (no session skip). Skip on click/Enter/Space/Escape; `visibilitychange` / `pageshow` re-kicks the canvas so backgrounded tabs still finish; wall-clock safety timeout prevents a stuck overlay. `prefers-reduced-motion` falls back to lobby still + short hold. Inspired by SAO-style tunnel-vision link-in (brand neon only; no SAO trademarks). Wireframe corridor prototype retained as reusable module [`assets/js/arcade-tunnel-scene.js`](../../assets/js/arcade-tunnel-scene.js) (`WhiteStudioArcadeTunnel`) for future music / dodge games — not loaded by hub intro.
