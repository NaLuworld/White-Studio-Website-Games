...
(function (global) {
  ...
  ColorChainNet.prototype.createRoom = async function (opts) {
    var res = await fetch(apiOrigin() + "/api/games/color-chain/rooms", {
      method: "POST",
  ...
  ColorChainNet.prototype.joinRoom = async function (opts) {
    var code = String(opts.code || "")
      .trim()
      .toUpperCase();
    var res = await fetch(apiOrigin() + "/api/games/color-chain/rooms/" + encodeURIComponent(code) + "/join", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        name: opts.name,
  ...
  ColorChainNet.prototype._scheduleReconnect = function () {
    var self = this;
